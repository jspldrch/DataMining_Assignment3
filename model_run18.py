# run18: 373+45 features (user context), 15 LGB + 9 XGB, augmentation, per-user norm
...
# data augmentation for rare classes (Gaussian noise after normalization)
AUG_CONFIG    = {2: 5, 4: 5, 5: 2, 3: 1}   # copies per class
AUG_NOISE_STD = 0.05
...
# user-contextual features: z-score of key stats relative to that user's baseline
# train: reference = original (non-augmented) windows; test: all test windows for user
N_CTX = 45   # stats9 x 5 key channels
...
print(f"  train: {X_tr_orig_feat.shape}  test: {X_te_feat.shape}")
...
print(f"  with user context: train {X_tr_aug_ctx.shape}  test {X_te_ctx.shape}")
