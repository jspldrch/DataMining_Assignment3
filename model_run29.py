...
def tilt_angle_features(mx, my, mz):
    tilt = np.arctan2(mz, np.sqrt(mx**2 + my**2))
    f = np.column_stack([
        tilt.mean(1), tilt.std(1), tilt.max(1)-tilt.min(1),
        tilt[:,:60].mean(1), tilt[:,-60:].mean(1),
        tilt[:,-60:].mean(1) - tilt[:,:60].mean(1),
    ])
    names = ["tilt_mean","tilt_std","tilt_range","tilt_first60","tilt_last60","tilt_trend"]
    return f.astype(np.float32), names
...
def axis_covcorr_features(mx, my, mz):
    ...
    for (a, b, nm) in [(mx, my, "xy"), (mx, mz, "xz"), (my, mz, "yz")]:
        ...
        parts += [cov, corr]; names += [f"mean_cov_{nm}", f"mean_corr_{nm}"]
    return np.column_stack(parts).astype(np.float32), names

def step_rhythm_features(std_mag):
    N, T = std_mag.shape; out = np.zeros((N, 4), dtype=np.float32)
    for n in range(N):
        v = std_mag[n]
        if np.std(v) < 1e-12: continue
        ...
        out[n, 0] = iv.mean(); out[n, 1] = iv.std()
        out[n, 2] = iv.std() / (iv.mean() + 1e-10); out[n, 3] = np.median(iv)
...
ax.set_title(f'CV Confusion Matrix — run29 (750 features, reg restored)\nOOF F1 = {baseline_f1:.4f}')
ax.set_xlabel('Predicted'); ax.set_ylabel('True')
plt.tight_layout()
plt.savefig(OUT_DIR / 'run29_confusion_matrix.png', dpi=150, bbox_inches='tight')
plt.close()
...
plt.suptitle(f'run29 — OOF Probability Matrices  (OOF F1={baseline_f1:.4f})', fontsize=11)
plt.tight_layout()
plt.savefig(OUT_DIR / 'run29_covariance_matrices.png', dpi=150, bbox_inches='tight')
plt.close()
print("  Saved: run29_covariance_matrices.png")
...
fig.suptitle('Per-class CV F1 — run29 (reg restored, red=C2 bottleneck)', fontsize=11)
plt.tight_layout()
plt.savefig(OUT_DIR / 'run29_per_class_f1.png', dpi=150, bbox_inches='tight')
plt.close()
...
FEATURE_GROUPS = [
    ...
    (716, 722, "tilt_angle"),
    (722, 728, "axis_cov_corr"),
    (728, 746, "mean_xyz_fft"),
    (746, 750, "step_rhythm"),
]
...
print(f"\n  {'Group':<34} {'N':>5}   {'% total':>8}   run28%")
print(f"  {'-'*60}")
run28_pcts = {
    "mean_xyz_axis_stat+diff": 8.7, "std_xyz_axis_stat+diff": 27.4,
    "acc_mag+std_mag_stat+diff": 11.9, "cross_magnitudes_stat+diff": 28.2,
    "sum_features_stat+diff": 5.0, "fft_peak_5ch": 2.4,
    "window_10win_5ch": 13.4, "tilt_angle": 0.3,
    "axis_cov_corr": 0.6, "mean_xyz_fft": 1.8, "step_rhythm": 0.4,
}
group_rows = []
for start, end, name in FEATURE_GROUPS:
    g   = avg_imp[start:end].sum(); pct = g / total * 100
    group_rows.append((name, end-start, g, pct))
    r28 = run28_pcts.get(name, 0)
    delta = f"({pct-r28:+.1f}pp)" if abs(pct-r28) > 0.2 else ""
    print(f"  {name:<34} {end-start:>5}   {pct:>7.1f}%   {r28:.1f}% {delta}")
...
axes[0].set_title('LGB Feature group importance — run29\n(orange=new, red=axis-mean)')
axes[0].invert_yaxis()

rf_imp = rf.feature_importances_
rf_rows = [(name, rf_imp[s:e].sum()/rf_imp.sum()*100)
           for s, e, name in FEATURE_GROUPS]
...
axes[1].set_title('RF Feature group importance — run29')
axes[1].invert_yaxis()

plt.suptitle('Feature group importance — run29 (750 features)', fontsize=11)
plt.tight_layout()
plt.savefig(OUT_DIR / 'run29_feature_importance.png', dpi=150, bbox_inches='tight')
plt.close()
print("  Saved: run29_feature_importance.png")
