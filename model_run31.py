"""
model_run31.py — run30 + lower learning rate (0.03→0.01) + wider patience (100→300)

Diagnosis from run27–run30:
  The early stopping point is identically ~160 iterations across run28/29/30 despite
  different features (716 vs 750). This is NOT a feature or regularisation issue —
  it is fixed by the learning rate and num_leaves combination.

  With lr=0.03, each boosting round takes a large step → the model converges to a
  local optimum at ~160 iterations. The val loss plateaus there, early stopping fires.

  With lr=0.01, each round takes a smaller step → the model explores more of the loss
  landscape before converging. Expected best_iter: ~400–600 (3× current ~160).
  Patience increased 100→300 proportionally, otherwise early stopping would still fire
  prematurely (~100 × lr_ratio = 300 equivalent rounds).

  The OOF→Kaggle gap is a consistent +0.060 across all runs, meaning a higher OOF
  will translate directly to a higher Kaggle score. To beat run27's 0.7895, OOF must
  exceed ~0.730.

Changes vs run30 (single axis of variation):
  learning_rate: 0.03 → 0.01
  early_stopping patience: 100 → 300
  final n_estimators: avg_best_iter × 1.10 (same formula, but expect higher result)
  Everything else identical to run30 (716 features, 5 LGB + 2 XGB + 1 RF).

Risk: longer runtime (~3× CV time). On Kaggle this stays within the 9-hour limit.
      On laptop: expect ~60–90 min total (vs ~20–30 min for run30).
"""
...
# ──────────────────────────────────────────────────────────────────────────────
# FEATURE HELPERS — identical to run27/30
# ──────────────────────────────────────────────────────────────────────────────
...
# ──────────────────────────────────────────────────────────────────────────────
# FEATURE EXTRACTION — 716 features, identical to run27/30
# ──────────────────────────────────────────────────────────────────────────────
def extract(X):
    N, T, _ = X.shape
...
print(f"  Features: {X_tr_feat.shape[1]}  (run27/30 exact)")
...
# ──────────────────────────────────────────────────────────────────────────────
# GROUPKFOLD CV
# KEY CHANGE: learning_rate 0.03→0.01, patience 100→300
# Smaller steps → model explores more of the loss landscape before converging.
# Expected best_iter: ~400–600 (vs ~160 in run27/28/29/30 at lr=0.03)
# ──────────────────────────────────────────────────────────────────────────────
print("\n" + "=" * 60)
print("GROUPKFOLD CV  (lr=0.01, patience=300 — exploring lower loss valley)")
print("=" * 60)
...
for fold, (tr_idx, va_idx) in enumerate(gkf.split(X_tr_feat, y_tr, groups=users), start=1):
    ...
    m = lgb.LGBMClassifier(
        ...
        n_estimators=5000,          # large upper bound; early stopping determines actual count
        learning_rate=0.01,         # 0.03→0.01: smaller steps, more iterations
    ...
    m.fit(
        ...
        callbacks=[
            lgb.early_stopping(stopping_rounds=300),  # 100→300: proportional to lr ratio
...
print(f"  run30 (lr=0.03): [179,171,159,124,173] avg=161 — should be ~3× higher here")
...
print(f"\nOOF macro F1 : {baseline_f1:.4f}  (run30: 0.7289, target: >0.730 to beat Kaggle 0.7895)")
print(f"Mean fold F1 : {np.mean(fold_f1s):.4f} ± {np.std(fold_f1s):.4f}")
print("Per-class F1 :", [f"{f:.3f}" for f in per_class_f1])
print(f"  C2 walk_down: {per_class_f1[2]:.3f}  (run30: 0.244)")
...
for i in range(6):
    ...
    flag = "  ← C2 bottleneck (run30: 0.20)" if i == 2 else ""
...
ax.set_title(f'CV Confusion Matrix — run31 (716 feat, lr=0.01, patience=300)\nOOF F1 = {baseline_f1:.4f}')
ax.set_xlabel('Predicted'); ax.set_ylabel('True')
plt.tight_layout()
plt.savefig(OUT_DIR / 'run31_confusion_matrix.png', dpi=150, bbox_inches='tight')
plt.close()
print("  Saved: run31_confusion_matrix.png")
...
plt.suptitle(f'run31 — OOF Probability Matrices  (OOF F1={baseline_f1:.4f})', fontsize=11)
plt.tight_layout()
plt.savefig(OUT_DIR / 'run31_covariance_matrices.png', dpi=150, bbox_inches='tight')
plt.close()
print("  Saved: run31_covariance_matrices.png")
...
print(f"\n  C2→C1 confusion: {soft_cm[2,1]:.4f}  (run30: 0.477)")
print(f"  C2 self-proba:   {soft_cm[2,2]:.4f}  (run30: 0.216)")

fig, ax = plt.subplots(figsize=(8, 6))
sns.heatmap(soft_cm, annot=True, fmt='.3f', cmap='Blues', ax=ax,
            xticklabels=CLASS_NAMES, yticklabels=CLASS_NAMES,
            linewidths=0.5, vmin=0, vmax=1)
ax.set_title('Soft Confusion (mean predicted prob per true class)\nrun31 — 716 feat, lr=0.01')
ax.set_xlabel('Predicted class probability'); ax.set_ylabel('True class')
plt.tight_layout()
plt.savefig(OUT_DIR / 'run31_soft_confusion.png', dpi=150, bbox_inches='tight')
plt.close()
print("  Saved: run31_soft_confusion.png")
...
fig.suptitle('Per-class CV F1 — run31 (lr=0.01, patience=300, red=C2)', fontsize=11)
plt.tight_layout()
plt.savefig(OUT_DIR / 'run31_per_class_f1.png', dpi=150, bbox_inches='tight')
plt.close()
print("  Saved: run31_per_class_f1.png")
...
for seed in range(5):
    m = lgb.LGBMClassifier(
        ...
        learning_rate=0.01,         # matches CV learning rate
        num_leaves=31, max_depth=-1,
        min_child_samples=20, subsample=0.85, colsample_bytree=0.85,
        reg_alpha=0.1, reg_lambda=1.0,
...
print(f"\n  {'Group':<34} {'N':>5}   {'% total':>8}   run30%")
print(f"  {'-'*60}")
group_rows = []
for start, end, name in FEATURE_GROUPS:
    g   = avg_imp[start:end].sum(); pct = g / total * 100
    group_rows.append((name, end-start, g, pct))
    r30 = run30_pcts.get(name, 0)
    delta = f"({pct-r30:+.1f}pp)" if abs(pct-r30) > 0.3 else ""
    print(f"  {name:<34} {end-start:>5}   {pct:>7.1f}%   {r30:.1f}% {delta}")
...
axes[0].set_title('LGB Feature group importance — run31\n(red = axis-specific mean, lr=0.01)')
axes[0].invert_yaxis()
...
axes[1].set_title('RF Feature group importance — run31')
axes[1].invert_yaxis()

plt.suptitle('Feature group importance — run31 (716 features, lr=0.01)', fontsize=11)
plt.tight_layout()
plt.savefig(OUT_DIR / 'run31_feature_importance.png', dpi=150, bbox_inches='tight')
plt.close()
print("  Saved: run31_feature_importance.png")
...
out_path = OUT_DIR / "submission_run31.csv"
...
print(f"\nRun comparison (OOF→Kaggle gap is consistently +0.060):")
print(f"  run27: ~0.730 OOF  0.7895 Kaggle  (716 feat, lr=0.03, patience=100)")
print(f"  run30:  0.729 OOF  0.7893 Kaggle  (716 feat, lr=0.03, patience=100, +RF)")
print(f"  run31:  {baseline_f1:.4f} OOF  ?.???? Kaggle  (716 feat, lr=0.01, patience=300, +RF)")
print(f"")
print(f"  Key diagnostic — best_iters:")
print(f"    run27–30: [~124–179] avg~161  (lr=0.03, plateau at ~160)")
print(f"    run31:    {best_iters}  avg={int(np.mean(best_iters))}")
print(f"    If avg > 300: lower lr successfully explored more landscape")
print(f"    If avg still ~160: the data has a hard ceiling at this capacity")
print(f"")
print(f"  C2 walk_down diagonal:  run30=0.20  run31={cm[2,2]:.2f}")
print(f"  C2→C1 confusion:        run30=0.477  run31={soft_cm[2,1]:.3f}")
