...
for seed in range(5):
    ...
    rf.fit(X_tr_sc, y_tr_aug)
    final_probas.append(rf.predict_proba(X_te_sc))
print(f"  RandomForest: {len(final_probas) - rf_start} models trained")
...
for start, end, name in FEATURE_GROUPS:
    grp_imp = avg_imp[start:end].sum()
    n_feats = end - start
    rows.append((name, n_feats, grp_imp, 100 * grp_imp / total_imp))

rows.sort(key=lambda x: -x[2])
print(f"\n  {'Group':<20} {'N':>5}  {'Importance':>12}  {'% total':>8}")
print("  " + "-"*50)
