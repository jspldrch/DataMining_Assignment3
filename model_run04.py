...
X_train_raw  = train_data["X"].astype(np.float32)       # (N, 300, 6)
...
def train_cnn(X_tr, y_tr, X_va=None, y_va=None, epochs=30, batch=64, lr=1e-3):
    cw = torch.tensor(
...
def extract_features(X: np.ndarray) -> np.ndarray:
    ...
    sl = np.zeros((N,C), dtype=np.float32)
    for c in range(C):
        sl[:,c] = (X[:,:,c]*t).sum(1)/(t**2).sum()
    parts.append(sl)
...
for fold in range(5):
    ...
    print("  Training CNN …")
    m = train_cnn(X_train_norm[tr_idx], y_train[tr_idx],
                  X_va=X_train_norm[va_idx], y_va=y_train[va_idx],
                  epochs=30, batch=64)
    cnn_loo_preds[va_idx] = predict_cnn(m, X_train_norm[va_idx]).numpy()
    ...
    if HAS_LGB:
        ...
        lgb_m.fit(X_tr_sc[tr_idx], y_train[tr_idx])
        lgb_loo_preds[va_idx] = lgb_m.predict(X_tr_sc[va_idx])
...
cnn_proba = predict_proba_cnn(final_cnn, X_test_norm)
...
if HAS_LGB:
    ...
    lgb_proba   = final_lgb.predict_proba(X_te_sc)
...
out_path   = OUT_DIR / "submission_run04.csv"
...
plt.tight_layout()
plt.savefig(OUT_DIR / "run04_confusion_matrix.png", dpi=180, bbox_inches="tight")
plt.close()
print("Saved: run04_confusion_matrix.png")
