...
def extract(X):
    ...
    # mag_mean intentionally omitted (gravity noise without normalization)
...
ax.set_title(f'CV Confusion Matrix — run25 (no augmentation)\nBaseline macro F1 = {baseline_f1:.4f}')
ax.set_xlabel('Predicted'); ax.set_ylabel('True')
plt.tight_layout()
plt.savefig(OUT_DIR / 'run25_confusion_matrix.png', dpi=150, bbox_inches='tight')
plt.close()
...
fig.suptitle('Per-class CV F1 — run25 (no augmentation)', fontsize=11)
plt.tight_layout()
plt.savefig(OUT_DIR / 'run25_per_class_f1.png', dpi=150, bbox_inches='tight')
plt.close()
...
print(f"\n  {'Group':<22} {'N':>4}   {'% total':>8}")
print(f"  {'-'*38}")
group_rows = []
for start, end, name in FEATURE_GROUPS:
    g = avg_imp[start:end].sum()
    group_rows.append((name, end-start, g, g/total*100))
    print(f"  {name:<22} {end-start:>4}   {g/total*100:>7.1f}%")

fig, ax = plt.subplots(figsize=(10, 5))
gr_sorted = sorted(group_rows, key=lambda x: x[2], reverse=True)
ax.barh([r[0] for r in gr_sorted], [r[3] for r in gr_sorted], color='#5bc0de')
ax.set_xlabel('% of total LightGBM gain')
ax.set_title('Feature group importance — run25 (no augmentation)')
ax.invert_yaxis()
plt.tight_layout()
plt.savefig(OUT_DIR / 'run25_feature_importance.png', dpi=150, bbox_inches='tight')
plt.close()
